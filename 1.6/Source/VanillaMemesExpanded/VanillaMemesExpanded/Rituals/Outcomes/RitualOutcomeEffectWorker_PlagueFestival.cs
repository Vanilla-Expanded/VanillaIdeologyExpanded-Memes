...
namespace VanillaMemesExpanded
{
	public class RitualOutcomeEffectWorker_PlagueFestival : RitualOutcomeEffectWorker_FromQuality
	{
		...
		public override void Apply(float progress, Dictionary<Pawn, int> totalPresence, LordJob_Ritual jobRitual)
		{
			...
			{
				IntVec3 c;
				CellFinder.TryFindRandomReachableCellNearPosition(jobRitual.selectedTarget.Cell, jobRitual.selectedTarget.Cell, jobRitual.Map, 2, TraverseParms.For(TraverseMode.NoPassClosedDoors, Danger.Deadly, false), null, null, out c);
				FilthMaker.TryMakeFilth(c, jobRitual.Map, ThingDefOf.Filth_CorpseBile);

			}
			InternalDefOf.Hive_Spawn.PlayOneShot(new TargetInfo(jobRitual.selectedTarget.Cell, jobRitual.Map, false));
		}
